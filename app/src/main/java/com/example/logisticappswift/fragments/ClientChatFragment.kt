...
import kotlinx.android.synthetic.main.fragment_client_chat.view.*
import kotlinx.android.synthetic.main.fragment_driver_chat.view.*
import kotlinx.android.synthetic.main.fragment_driver_chat.view.driver_chat_recyclerview
...
class ClientChatFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater!!.inflate(R.layout.fragment_client_chat, container, false)
        fetchUser(view)
        return view
    }

    private fun fetchUser(view:View){
        val ref = FirebaseDatabase.getInstance().getReference("/users/driver")
        ref.addListenerForSingleValueEvent(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                snapshot.children.forEach {
                    ...
                    if(user != null){
                        adapter.add(ClientNewMessage(user))
                    }
                }
                adapter.setOnItemClickListener { item, view ->
                    val intent = Intent(view.context, ChatLogActivity :: class.java)
                    val userItem = item as ClientNewMessage
                    intent.putExtra("USER_KEY", userItem.user )
                    startActivity(intent)
                }
                view.client_chat_recyclerview.adapter = adapter
            }
    ...
    class ClientNewMessage(val user:User): Item<ViewHolder>(){
